...
echo "========================================================================"
echo "Multi-Project Verification & Health Check"
echo "Workload Project (Project A): ${WORKLOAD_PROJECT_ID}"
echo "Consumer Project (Project B): ${CONSUMER_PROJECT_ID}"
echo "========================================================================"

AUTH_TOKEN="$(gcloud auth print-access-token)"

echo ""
echo "--- [1/5] Checking Workload Project Backend & ALB Health (${WORKLOAD_PROJECT_ID}) ---"
echo "Backend service health:"
gcloud compute backend-services get-health "${PROJECT_NAME}-backend-service" \
  --region="${GKE_REGION}" \
  --project="${WORKLOAD_PROJECT_ID}" --format="table(status.healthStatus[0].ipAddress,status.healthStatus[0].healthState)" || echo "Warning: Backend service health query failed."
...
echo ""
echo "--- [2/5] Checking Consumer Project Agent Gateway (${CONSUMER_PROJECT_ID}) ---"
...
echo ""
echo "--- [3/5] Checking Consumer Project Agent Registry (${CONSUMER_PROJECT_ID}) ---"
...
if [ -z "${DISCOVERY_ENGINE_AGENT_ID}" ] || [ "${DISCOVERY_ENGINE_AGENT_ID}" == "null" ]; then
  ...
  echo "Run ./scripts/register_multi_project.sh first."
  exit 1
fi
...
echo "Sending request to: https://discoveryengine.googleapis.com/v1alpha/projects/${CONSUMER_PROJECT_NUM}/.../assistants/default_assistant:streamAssist"
echo ""
...
curl -N -X POST \
  ...
  "https://discoveryengine.googleapis.com/v1alpha/projects/${CONSUMER_PROJECT_NUM}/locations/global/collections/default_collection/engines/${ENGINE_ID}/assistants/default_assistant:streamAssist" \
  -d '{
    "query": {
...
echo ""
echo "========================================================================"
echo "Multi-Project Validation Complete!"
echo "========================================================================"
