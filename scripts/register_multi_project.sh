...
WORKLOAD_PROJECT_ID="${WORKLOAD_PROJECT_ID:-${PROJECT_ID:-your-workload-project-id}}"
CONSUMER_PROJECT_ID="${CONSUMER_PROJECT_ID:-${DNS_PROJECT_ID:-your-consumer-project-id}}"
CONSUMER_PROJECT_NUM="${CONSUMER_PROJECT_NUM:-${PROJECT_NUM:-your-consumer-project-num}}"
...
echo "========================================================================"
echo "Registering A2A Agent in Multi-Project Setup"
echo "Workload Project (GKE/ALB): ${WORKLOAD_PROJECT_ID}"
echo "Consumer Project (GE/Gateway/Registry): ${CONSUMER_PROJECT_ID} (${CONSUMER_PROJECT_NUM})"
...
echo ""
echo "Step 1: Registering / Updating Service in Agent Registry (${CONSUMER_PROJECT_ID})..."
if gcloud alpha agent-registry services describe "${PROJECT_NAME}" --location="${GKE_REGION}" --project="${CONSUMER_PROJECT_ID}" >/dev/null 2>&1; then
  ...
  gcloud alpha agent-registry services create "${PROJECT_NAME}" \
    ...
    --description="GKE Hosted A2A Service in ${WORKLOAD_PROJECT_ID}" \
...
curl -s -X PATCH \
  ...
  "https://discoveryengine.googleapis.com/v1alpha/projects/${CONSUMER_PROJECT_NUM}/locations/global/collections/default_collection/engines/${ENGINE_ID}?updateMask=agentGatewaySetting" \
  -d '{
    "agentGatewaySetting": {
      "defaultEgressAgentGateway": {
        "name": "projects/'"${CONSUMER_PROJECT_ID}"'/locations/'"${GATEWAY_REGION}"'/agentGateways/'"${GATEWAY_NAME}"'"
      }
    }
...
cat << 'EOF' > "${AGENT_CARD_FILE}"
{
  ...
  "description": "Greets users and provides personalized greetings via GKE and cross-project Agent Gateway",
...
IMPORT_RESPONSE=$(curl -s -X POST \
  ...
  -H "X-Goog-User-Project: ${CONSUMER_PROJECT_ID}" \
  -H "Content-Type: application/json" \
  "https://discoveryengine.googleapis.com/v1alpha/projects/${CONSUMER_PROJECT_NUM}/locations/global/collections/default_collection/engines/${ENGINE_ID}/assistants/default_assistant/agents" \
  -d '{
    "displayName": "Hello World GKE Multi-Project",
    "description": "Greets users via GKE in '"${WORKLOAD_PROJECT_ID}"' and Agent Gateway in '"${CONSUMER_PROJECT_ID}"'",
    "importedAgent": {
...
echo "========================================================================"
echo "Multi-Project Agent Registration Completed Successfully!"
